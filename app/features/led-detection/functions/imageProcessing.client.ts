...
export async function calibrateUsingCamera(
  ...
  const ledPositions = new Array<ILedPos>(1);
  ledPositions.fill({ x: 0, y: 0 }, 0, ledPositions.length);

  const colors = new Array<ILedColor>(1);
  colors.fill({ r: 0, g: 0, b: 0 }, 0, colors.length);
  console.dir({ ledPositions }, { depth: null });
  ...
  for (let ledIdx = 0; ledIdx < ledPositions.length; ledIdx++) {
    ...
    await delay(10000);
    if (promiseObj !== null) {
      console.log("awaiting custom promise");
  ...
  console.dir({ ledPositions }, { depth: Infinity });
  const context = canvas.getContext("2d");

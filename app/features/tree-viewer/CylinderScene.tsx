import { useShallow } from "zustand/react/shallow";
import useEditorStore, {
  ByGroupType,
  CanvasExport,
  TimelineSelectedGroupIdType,
} from "../tree-editor/state/useEditorStore";
import { useEffect, useMemo, useRef } from "react";
import { useTexture } from "@react-three/drei";
import LightSphere, { SphereProps } from "./LightSphere";
import pos2 from "./pos2";
import * as THREE from "three";

const points = [new THREE.Vector3(0, -10, 0), new THREE.Vector3(0, 10, 0)];

const TRANSPARENT_IMG =
  "data:image/png;base64,";

const defaultImage = "/imgs/test_pattern.png";

interface GetSortedCanvasExportsArgs {
  canvasExports: CanvasExport[] | null;
  attributesByGroup: ByGroupType;
  timelineSelectedGroupId: TimelineSelectedGroupIdType;
}

const getCurrentCanvasExports = ({
  canvasExports,
  attributesByGroup,
  timelineSelectedGroupId,
}: GetSortedCanvasExportsArgs) => {
  const groupIds = Object.keys(attributesByGroup);
  const allCanvasExports = groupIds.reduce((acc, groupId) => {
    if (groupId === timelineSelectedGroupId && canvasExports) {
      acc.push(...canvasExports);
      return acc;
    }
    const canvasExportsByGroup = attributesByGroup[groupId].canvasExports;
    if (canvasExportsByGroup) {
      acc.push(...canvasExportsByGroup);
    }
    return acc;
  }, [] as CanvasExport[]);

  return allCanvasExports;
};

const getTextureUrls = ({
  canvasExports,
  allCanvasExports,
  isTimelinePlaying,
}: {
  canvasExports: CanvasExport[] | null;
  allCanvasExports: CanvasExport[];
  isTimelinePlaying: boolean;
}) => {
  const defaultResult = [
    canvasExports?.[0]?.canvasCylinderImgUrl ?? defaultImage,
  ];
  if (isTimelinePlaying) {
    const initial =
      allCanvasExports.map((obj) => obj.canvasCylinderImgUrl) ?? [];
    return [TRANSPARENT_IMG, ...initial];
  }

  return defaultResult;
};

const getTextureIndex = ({
  timelineCoarseTime,
  allCanvasExports,
  isTimelinePlaying,
}: {
  timelineCoarseTime: number;
  allCanvasExports: CanvasExport[];
  isTimelinePlaying: boolean;
}) => {
  if (isTimelinePlaying) {
    const data = allCanvasExports;
    const max = allCanvasExports.reduce((acc, datum) => {
      return Math.max(acc, datum.end);
    }, 0);
    for (let i = 0; i < data.length; i++) {
      const { start, end } = data[i];
      if (
        timelineCoarseTime >= start &&
        (timelineCoarseTime < end || timelineCoarseTime == max)
      ) {
        return i + 1;
      }
    }
  }

  return 0;
};

function getTextureColor(
  context: CanvasRenderingContext2D,
  u: number,
  v: number,
  sampleContext2: CanvasRenderingContext2D | null,
): THREE.Color {
  const x = Math.floor(u * context.canvas.width);
  const y = Math.floor((1 - v) * context.canvas.height);
  const pixel = context.getImageData(x, y, 1, 1).data;
  if (sampleContext2 !== null) {
    sampleContext2.strokeStyle = "purple";
    sampleContext2.lineWidth = 3;
    sampleContext2.strokeRect(x - 2, y - 2, 4, 4);
  }

  const r = pixel[0];
  const g = pixel[1];
  const b = pixel[2];

  return new THREE.Color(`rgb(${r}, ${g}, ${b})`);
}

const getSpheres = ({
  textureImage,
  cylinderHeight,
}: {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  textureImage: any;
  cylinderHeight: number;
}) => {
  const generatedSpheres: SphereProps[] = [];

  const sampleCanvas = document.getElementById(
    "testCanvas",
  ) as HTMLCanvasElement;
  const sampleContext = sampleCanvas.getContext("2d");
  if (sampleContext && textureImage) {
    sampleCanvas.width = textureImage.width;
    sampleCanvas.height = textureImage.height;
    sampleContext.drawImage(textureImage, 0, 0);
  }

  const sampleCanvas2 = document.getElementById(
    "testCanvas2",
  ) as HTMLCanvasElement;
  const sampleContext2 = sampleCanvas2.getContext("2d");
  if (sampleContext2 && textureImage) {
    sampleCanvas2.width = textureImage.width;
    sampleCanvas2.height = textureImage.height;
    sampleContext2.drawImage(textureImage, 0, 0);
  }

  pos2.forEach(({ x, y, z, angle }, index) => {
    const u = (360 - (((Math.atan2(z, x) * 180) / Math.PI + 270) % 360)) / 360;
    const v = (y + cylinderHeight / 2) / cylinderHeight;

    const color = sampleContext
      ? getTextureColor(sampleContext, u, v, sampleContext2)
      : new THREE.Color(0xffffff);

    // Calculate the sample point on the cylinder surface
    const sampleX = 6 * Math.cos(angle);
    const sampleZ = 6 * Math.sin(angle);
    const samplePoint: [number, number, number] = [sampleX, y, sampleZ];
    generatedSpheres.push({
      position: [x, y, z],
      color,
      samplePoint,
      id: index,
    });
  });

  return generatedSpheres;
};

const CylinderScene = () => {
  //

  const {
    attributesByGroup,

    canvasExports,

    timelineCoarseTime,
    timelinePlayingState,
    timelineSelectedGroupId,

    treeViewerCylinderOpacity,
  } = useEditorStore(
    useShallow((state) => ({
      attributesByGroup: state.attributesByGroup,

      canvasExports: state.canvasExports,

      timelineCoarseTime: state.timelineCoarseTime,
      timelinePlayingState: state.timelinePlayingState,
      timelineSelectedGroupId: state.timelineSelectedGroupId,

      treeViewerCylinderOpacity: state.treeViewerCylinderOpacity,
    })),
  );
  const isTimelinePlaying = timelinePlayingState === "playing";

  const allCanvasExports = useMemo(
    () =>
      getCurrentCanvasExports({
        canvasExports,
        attributesByGroup,
        timelineSelectedGroupId,
      }),
    [attributesByGroup, canvasExports, timelineSelectedGroupId],
  );

  const urlsToUse = useMemo(() => {
    const result = getTextureUrls({
      canvasExports,
      allCanvasExports,
      isTimelinePlaying,
    });
    return result;
  }, [allCanvasExports, canvasExports, isTimelinePlaying]);

  const texturesIndex = useMemo(() => {
    const result = getTextureIndex({
      timelineCoarseTime,
      allCanvasExports,
      isTimelinePlaying,
    });
    return result;
  }, [allCanvasExports, isTimelinePlaying, timelineCoarseTime]);

  const textures = useTexture(urlsToUse);

  const texture = textures[texturesIndex];

  const cylinderHeight = 20;
  const cylinderRef = useRef<THREE.Mesh>(null);
  const lineRef = useRef<THREE.BufferGeometry>(null);

  const spheres = useMemo(
    () => getSpheres({ textureImage: texture.image, cylinderHeight }),
    [texture.image],
  );

  useEffect(() => {
    if (lineRef.current) {
      lineRef.current.setFromPoints(points);
    }
  }, []);

  return (
    <>
      <mesh ref={cylinderRef} position={[0, 0, 0]}>
        <cylinderGeometry args={[5, 5, cylinderHeight, 64, 1, true]} />
        <meshBasicMaterial
          map={texture}
          side={THREE.DoubleSide}
          transparent
          opacity={treeViewerCylinderOpacity}
        />
      </mesh>
      <line>
        <bufferGeometry ref={lineRef} />

        <lineBasicMaterial color={0x00ff00} />
      </line>

      {spheres.map((sphere, index) => {
        return <LightSphere key={index} index={index} sphere={sphere} />;
      })}
    </>
  );
};

export default CylinderScene;
